                ...
                Scaffold(
                    ...
                    NavHost(navController = navController, startDestination = "home"){
                        composable(route = "home") {
                            Shop(navController = navController, onSubmit = { product ->
                                sharedViewModel.setProduct(product)
                                navController.navigate("orderDetails")
                            }, modifier = Modifier.padding(innerPadding))
                        }
                        composable(route = "orderDetails"){
                            OrderDetails(sharedViewModel = sharedViewModel, navController = navController, modifier = Modifier.padding(innerPadding))
                        }
                    }
...
fun Shop(navController: NavController, onSubmit: (ProductModel) -> Unit, modifier: Modifier = Modifier){
    ...
    Column(modifier = Modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.Center, horizontalAlignment = Alignment.Start) {
        Image(painter = painterResource(R.drawable.image), contentDescription = null, modifier = Modifier.padding(bottom = 10.dp).fillMaxWidth())
        Text("ชานมไข่มุก", fontSize = 20.sp, fontWeight = FontWeight.Bold)
...
fun OrderDetails(sharedViewModel: SharedViewModel, navController: NavController, modifier: Modifier = Modifier){
    ...
    Column(modifier = Modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.Center, horizontalAlignment = Alignment.Start) {
        Image(painter = painterResource(R.drawable.image,), contentDescription = null, modifier = Modifier.fillMaxWidth().padding(bottom = 10.dp))
...
fun Greeting(modifier: Modifier = Modifier) {
    ...
    Column(modifier = modifier
        ...
        Text("ผบคำตอบของคุณเลือกคือ: $selectedOption", fontSize = 18.sp, fontWeight = FontWeight.Bold, textAlign = TextAlign.Center, modifier = modifier.fillMaxWidth())
    }
}